app.component("home-feature", {
    ...
    async created() {
        ...
        const opportunityAPI = new API("opportunity"); // Modificação Mapa da Inovação

        const query = this.query;

        query["@select"] =
            "id,name,shortDescription,location,terms,seals,singleUrl";
        ...
        Promise.all(promises).then((values) => {
            ...
            this.opportunities = values[3] ?? []; // Modificação Mapa da Inovação
        });
    },
    ...
    data() {
        return {
            ...
            spaces: [],
            projects: [],
            opportunities: [], // Modificação Mapa da Inovação
    ...
    computed: {
        entities() {
            const entities = [
                ...
                ...this.agents,
                ...this.projects,
                ...this.opportunities, // Modificação Mapa da Inovação
            ].sort((a, b) => {
                if (a.name > b.name) {
                    return 1;
    ...
    methods: {
        enabledEntities() {
            return (
                this.global.enabledEntities.agents ||
                this.global.enabledEntities.spaces ||
                this.global.enabledEntities.projects ||
                this.global.enabledEntities.opportunities // Modificação Mapa da Inovação
            );
        },
    },
});
